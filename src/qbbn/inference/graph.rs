...
use crate::qbbn::{
    common::graph::InferenceGraph,
    model::{
        choose::extract_backimplications_from_proposition,
        objects::{ImplicationFactor, Proposition, PropositionGroup},
    },
};
...
fn reverse_prune_duplicates(raw_order: &[(i32, PropositionNode)]) -> Vec<PropositionNode> {
    let mut seen = HashSet::new();
    let mut result = vec![];
    for (_depth, node) in raw_order.iter().rev() {
...
fn create_bfs_order(proposition_graph: &PropositionGraph) -> Vec<PropositionNode> {
    ...
    reverse_prune_duplicates(&buffer)
}
