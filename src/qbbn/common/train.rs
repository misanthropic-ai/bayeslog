...
use super::model::FactorContext;
use crate::qbbn::common::proposition_db::RedisBeliefTable;
use crate::qbbn::model::choose::extract_backimplications_from_proposition;
...
impl TrainingPlan {
    ...
    pub fn maybe_add_to_training(
        ...
        if is_training {
            self.add_proposition_to_queue(connection, "training_queue", proposition)
        } else {
            Ok(())
        }
    }
    ...
    pub fn maybe_add_to_test(
        ...
        if is_test {
            self.add_proposition_to_queue(connection, "test_queue", proposition)
        } else {
            Ok(())
        }
    }
...
pub fn setup_and_train(
    ...
    let _model_spec = "dummy_model_spec".to_string();
